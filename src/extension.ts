...
export function activate(context: vscode.ExtensionContext) {
	...
	// Initialize native voice recorder (uses naudiodon - no external deps!)
	const voiceRecorder = new VoiceRecorder({
		sampleRate: 16000,
		channels: 1,
		silenceThreshold: 500,  // Amplitude threshold for silence detection (0-32767)
		silenceDuration: 1500,  // 1.5 seconds of silence before auto-stop
		debug: false
	});
	provider.setVoiceRecorder(voiceRecorder);

	// Register voice command - uses native recording if available, falls back to browser popup
	const voiceDisposable = vscode.commands.registerCommand('claude-code-chat.startVoice', async () => {
		console.log('Voice input command executed!');
		...
		if (nativeAvailable) {
			...
			try {
				...
				if (transcript && transcript.trim()) {
					...
					provider.sendVoiceMessage(transcript);
					vscode.window.showInformationMessage(`Voice: "${transcript.substring(0, 50)}..."`);
				} else {
					vscode.window.showWarningMessage('Could not transcribe audio. Please try again.');
				}
			} catch (error) {
	...
	// Register stop voice command
	const stopVoiceDisposable = vscode.commands.registerCommand('claude-code-chat.stopVoice', () => {
		console.log('Stop voice command executed');
		voiceRecorder.stopRecording();
	});
...
class ClaudeChatProvider {
	...
	public sendVoiceMessage(transcript: string): void {
		if (transcript.trim()) {
			// Send the message to Claude (don't show in input, just send directly)
			this._sendMessageToClaude(transcript);
		}
	}
	...
	/**
	 * Transcribe audio captured directly from webview microphone
	 */
	...
	private _handleWebviewMessage(message: any) {
		switch (message.type) {
			...
			case 'transcribeAudio':
				...
				this._transcribeWebviewAudio(message.audio, message.mimeType);
				return;
			case 'newSession':
				this._newSession();
				return;
	...
	private async _processJsonStreamData(jsonData: any) {
		console.log('>>> JSON STREAM DATA type:', jsonData.type);
		switch (jsonData.type) {
			...
			case 'assistant':
				...
				console.log('jsonData.message:', JSON.stringify(jsonData.message).substring(0, 200));
				if (jsonData.message && jsonData.message.content) {
					...
					console.log('Processing assistant message content, items:', jsonData.message.content.length);
					for (const content of jsonData.message.content) {
						console.log('Content item type:', content.type);
						if (content.type === 'text' && content.text.trim()) {
							console.log('TEXT CONTENT FOUND, calling speakResponse');
							...
							this.speakResponse(content.text.trim());
